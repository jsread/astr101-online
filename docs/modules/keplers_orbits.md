# Kepler's Orbits

- Tycho’s Observations
- Ellipticity
- Kepler’s First Law
- Asteroids and Comets

## Tycho’s Observations



Tycho Brahe 1546-1601, Denmark. Helped refute celestial sphere model by finding “new stars” or novae/supernovae. Made the most accurate (to arcminutes) astronomical observations of the time. Had a weird hypothesis fot the plantary system.

![Portrait of Tycho Brahe](img/standard_compressed_Tycho_Brahe_1_.jpeg)

![Tycho's observations of new stars](img/tycho-new-stars.png)
![Tycho's proposed planetary system](img/tycho-hypothesis-system.png)


* 50 years after Copernicus, we have:
    * Galileo’s telescope observations of planets and moons
    * Tycho’s observations by eye of planets accurate to 1 arc minute
* BOTH the Ptolmeic and the Copernican models FAIL to match Tycho’s observational data perfectly


## Kepler's Proposal

“If I had believed that we could ignore these eight minutes, I would have patched up my hypothesis accordingly. But since it was not permissible to ignore them, those eight minutes pointed the road to a complete reformation of astronomy.”  - Johannes Kepler

Johannes Kepler, (1571-1630) German mathematician and astronomer. In 1609, developed the laws of planetary motion, “Kepler’s Laws”. 
![Portrait of Kepler](img/johannes-kepler-1610-149790-1024.jpg)

He used them to predict the apparent motion of Mars.
![Kepler's diagram of Mars' orbit.](img/kepler-diagram.jpg)

## Kepler's First Law

![Diagram of an ellipse. A pencil traces a shape with a string connected to two foci. The Sun is at one focus. The Major axis is the longest distance across, semi-major is half of that, the Minor axis is the smallest. The Planet follows the ellipse. Perihelion is closest to the Sun and aphelion is farthest.](img/kepler-first-diagram.png)

The orbit of each planet is an ellipse with the Sun at one focus

- Major axis: longest distance across
- Minor axis: longest distance across
- Perhelion: closest to Sun
- Aphelion: farthest from Sun


## Ellipses and Eccentricity

<figure>
  <video controls width="100%">
    <source src="../img/EllipseEccentricity.mp4" type="video/mp4">
    Your browser does not support the video tag.
  </video>
  <figcaption>Video showing the ellipse that comes from two foci as the foci move toward and away from each other.</figcaption>
</figure>

![Ellipses of varying eccentricity. e=0 is a circle. e=0.23 still looks a lot like a circle. e=0.43 is clearly an ellipse. e=0.77 is pretty squashed.](img/varying-eccentricity.png)

Earth's orbital eccentricity is 0.0167.

## Planets and Asteroids

The inner planets have fairly circular orbits.
![Inside the asteroid belt, Mars, Earth, Venus, and Mercury have orbits that look fairly circular but are a bit eccentric.](img/inner-orbits-asteroids.png)

While asteroids are mostly in the asteroid belt, they can have orbits that come close to (Near Earth) or cross (Potentially Hazardous) Earth's orbit. 
![Coming in from the asteroid belt, a Near Earth Asteroid has an elliptic orbit that comes near Earth's orbit. A Potentially Hazardous Asteroid has an orbit that crosses Earth's orbit.](img/inner-orbits-asteroids-2.png)

## Comets


<figure>
  <video controls width="100%">
    <source src="../img/comet-orbit.mp4" type="video/mp4">
    Your browser does not support the video tag.
  </video>
  <figcaption>A comet moves in from far away from the Sun on an eccentric orbit. The tail of the comet always points away from the Sun (not in the direction of motion)</figcaption>
</figure>


### Halley's Comet
![rocky nucleus of a comet with a glowing tail](img/Giotto_halley_br.jpg)
*Source: [Halley Multicolor Camera Team, Giotto Project, ESA
via NASA page on Halley's Comet](https://science.nasa.gov/solar-system/comets/1p-halley/)*

Comets in very elliptic orbits spend most of their time far away from the sun and return to Earth rarely.
![A map of Halley's Comet's orbit compared to Neptune, Uranus, Saturn, Jupiter, and the inner planets. It passes Earth in 1986, and then is beyond neptun in 1995, and far away from Earth in 2024. It returns in 2061.](img/comet-halley.png)

### Comet ATLAS

This path does not return in an orbit

This is comet ATLAS, which passed near Earth in 2025. On November 10, 2025, Hubble captured the inset image of the fragmenting comet.
![Diagram showing comet K1’s path. With the Sun near middle right of image, truncated nearly circular orbits of Mercury, Venus, Earth, and Mars appear against black background. K1’s tight parabolic curve, marked by solid, light blue curving line, illustrates how K1 swooped toward the Sun from above. It curved around the Sun, coming closest inside Mercury’s orbit, and continued its outbound journey.](img/comet-atlas-heic2606c.jpg)
*Source: [NASA, ESA, R. Crawford (STScI)](https://esahubble.org/images/heic2606c/)*

## Check your understanding

<quiz>
Which of these shapes has the largest eccentricity?
(judged by the outline from this view)

- [] ![an egg a bit more tall than wide](../img/egg.jpg){ width="100" }
- [] ![watermelon more wide than tall](../img/watermelon.jpg){ width="100" }
- [] ![an oval window much taller than wide](../img/window.jpg){ width="70" }
- [x] ![a circular basketball](../img/basketball.jpg){ width="100" }
</quiz>

<quiz>
The eccentricity of the Earth’s orbit is e = 0.0167

Which of the three orbits shown below
most closely matches the shape of the 
Earth’s orbit around the Sun, 
seen from directly above the orbital plane?

![A is a circle. B is notably more squashed. C is very squashed.](../img/earth-orbit-options.png)

- [x] (A)
- [] (B)
- [] (C)
</quiz>




<quiz>
Which of these animations, showing a planet orbiting around a star (from directly over the system) obeys Kepler’s First law?

![A is squashed ellipse with the sun at the center. B is a squashed ellipse with the sun closer to one side. C is a double loop structure. D is a circle with the sun closer to one side. ](../img/orbit-options.gif) 
- [] (A)
- [x] (B)
- [] (C)
- [] (D)
</quiz>